...
use super::upstream_support_and_manual_jobs::*;
    ...
    #[tokio::test]
    async fn body_gc_index_ensure_defers_when_instance_schema_permit_is_busy() {
        let db_path = temp_db_path("maintenance-worker-body-gc-index-schema-permit");
        let db_str = db_path.to_string_lossy().to_string();
        let proxy = TavilyProxy::with_endpoint(Vec::<String>::new(), DEFAULT_UPSTREAM, &db_str)
            .await
            .expect("proxy created");
        ...
        let _schema_permit = match state.proxy.admit_request_logs_gc_schema() {
            ...
            tavily_hikari::SqliteSchemaAdmissionOutcome::Deferred { reason } => {
                panic!("hold the instance-local schema permit, deferred={reason}")
            }
        };
        let job_id = state
            .proxy
        ...
        let jobs: Vec<(String, Option<String>)> =
            sqlx::query_as("SELECT status, message FROM scheduled_jobs ORDER BY id ASC")
        .fetch_all(&pool)
        .await
        .expect("read deferred schema-index jobs");
        assert_eq!(jobs.len(), 2);
        assert_eq!(jobs[0].0, "success");
        assert_eq!(jobs[0].1.as_deref(), Some("deferred=schema_busy"));
        assert_eq!(jobs[1].0, "queued");
        assert_eq!(jobs[1].1, None);
