...
impl KeyStore {
    ...
    async fn apply_admin_alert_canonical_groups_migration(&self) -> Result<(), ProxyError> {
        // This local read model is derived solely from the complete alert
        // projection. Startup creates empty state; the warm controller fills
        // it in bounded slices and publishes a source-fenced generation.
        sqlx::query(
