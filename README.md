...
1. Create an access token (for example `th-abcd-1234567890ab`, format `th-<id>-<secret>`) from the Tavily Hikari **user dashboard** and copy it.
...
5. Set **API key** to the Hikari access token from step 1 (the full `th-<id>-<secret>` value), **not** your Tavily official API key.
