...
- The same 60-second window separately aggregates canonical Events indexed reads, canonical Groups
  build slices/publishes/defers, and Key-observation identity reuses/misses. These counters describe
  source-fence progress only; they never emit a Key, token, source identity, SQL, or payload.
