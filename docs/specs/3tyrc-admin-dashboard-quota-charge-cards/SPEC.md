...
- `今日` 与 `本月` 总览块需要使用小时请求窗口渲染累计用量背景图，当前窗口与昨日同刻窗口叠加显示；本月图使用月累计总量作为今日小时增量的基线，不伪造月内历史逐日形状；图表必须铺满整块卡片背景且不参与交互。
...
- `今日` 与 `本月` 总览块都有背景累计折线图，今日块叠加今日与昨日同刻，本月块叠加月累计基线后的今日小时进度与对照进度；前景卡片仍保持数字可读。
