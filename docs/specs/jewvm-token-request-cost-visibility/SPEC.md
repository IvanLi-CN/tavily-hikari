...
- [ ] M1: 新 spec 与 README 索引落地，冻结字段/文案/范围边界
- [ ] M2: 后端 token logs / page / events 透传 `business_credits` 并补齐测试
- [ ] M3: Token detail 桌面/移动/详情新增 `Charged Credits` 展示并调稳布局
- [ ] M4: 全站同类日志视图 `MCP Status` / `MCP` 正名为 `Tavily Status` / `Tavily`
