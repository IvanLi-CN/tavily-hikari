...
- Treat the default alert catalog, events page `1/20`, and groups page `1/20` as pinned canonical
  ...
  check; a foreground checkout or waiter produces a typed defer. After `120s` without a complete
  canonical generation publish, the controller retries a liveness slot every `5s`; partial slices remain
  observable but do not reset the anchor. A slot covers one logical canonical stage and its fenced
...
- When default Groups aggregation cannot meet that budget from the generic projection CTE, build a
  ...
  source set. Source pages read at most 2,000 rows per independently-deadlined session and partition
  reads at 25 rows for deadline and cancellation checks. Source rows are committed in adaptive batches of at most 100 rows and
