...
- When default Groups aggregation cannot meet that budget from the generic projection CTE, build a
  ...
  small background write batches without gating active-generation publication; require the bounded
  backlog to drain before staging a later replacement. Do not use the model for filtered queries or
  replicate it through HA.
