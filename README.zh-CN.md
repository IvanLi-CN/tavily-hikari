...
1. 在 Tavily Hikari **用户总览页**中创建访问令牌（例如 `th-abcd-1234567890ab`，格式 `th-<id>-<secret>`），复制该 token。
...
5. 将 **API 密钥 / API key** 填写为步骤 1 中复制的 Hikari 访问令牌（完整的 `th-<id>-<secret>`），而不是 Tavily 官方 API key。
