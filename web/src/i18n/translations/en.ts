...
export const EN: TranslationShape = {
    ...
    public: {
      ...
      freshness: {
        fresh: 'Showing the latest available metrics.',
        stale: 'Showing usable but slightly older metrics while the backend catches up.',
        degraded: 'Showing a degraded view; some statistics are temporarily unavailable.',
        pendingRollups: 'Pending rollups still need to be absorbed, so this view may lag slightly.',
        sqliteContention: 'SQLite is contending on a write lock, so the latest successful metrics are shown first.',
        coldStart: 'Cold start fallback is active. Zero values here do not mean real traffic is zero.',
        optionalFeedFailure: 'Some optional feeds are unavailable right now, but the core public metrics still load.',
      },
