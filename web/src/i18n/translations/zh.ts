...
export const ZH: TranslationShape = {
    ...
    public: {
      ...
      freshness: {
        fresh: '当前展示的是最新统计数据。',
        stale: '当前展示的是可用但稍旧的数据，后台会继续自动追平。',
        degraded: '当前展示的是退化视图，部分统计可能暂不可用。',
        pendingRollups: '仍有待吸收的聚合写入，当前展示的是可用但稍旧的数据。',
        sqliteContention: 'SQLite 正在竞争写锁，当前先展示最近一次成功统计。',
        coldStart: '冷启动期间暂时只能展示退化视图，零值不代表真实数据。',
        optionalFeedFailure: '部分辅助数据源暂不可用，核心公开统计仍可查看。',
      },
      adminButton: '打开管理员面板',
    ...
    admin: {
      ...
      dashboard: {
        ...
        freshnessStale: '当前展示的是可用但稍旧的数据，后台会自动继续追平。',
        ...
        freshnessColdStart: '冷启动期间暂时只能展示退化视图，零值不代表真实数据。',
        freshnessOptionalFeedFailure: '部分辅助数据源暂不可用，核心摘要仍然可读。',
        todayTitle: '今日',
