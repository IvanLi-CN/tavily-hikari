...
function normalizeRequestLogFacets(
  value?: ServerRequestLogFacets,
): RequestLogFacets {
  return {
...
function normalizeRequestLogsPage(
  value: ServerRequestLogsPage,
): RequestLogsPage {
  return {
    items: value.items ?? [],
    total: value.total ?? 0,
    ...
    requestKindOptions:
      value.requestKindOptions ?? value.request_kind_options ?? [],
...
function appendRequestLogsPageFilters(
  ...
  if (operationalClass && operationalClass !== "all")
...
export type ForwardProxyProgressNodeStatus =
  | "pending"
  | "probing"
  | "ok"
  | "failed";
...
function extractErrorMessage(
  ...
  const err = new Error(
    (fallbackBody != null && fallbackBody.trim().length > 0
      ? fallbackBody
...
function parseForwardProxySseChunk(
  chunk: string,
): ForwardProxyProgressEvent | null {
...
async function requestForwardProxyProgress<T>(
  ...
  while (true) {
    ...
    while (boundaryIndex >= 0) {
      ...
      buffer = buffer.slice(
        boundaryIndex + (buffer[boundaryIndex] === "\r" ? 4 : 2),
      );
      ...
      if (event) {
        ...
        if (event.type === "error") {
          throw new Error(
            event.message ||
              event.detail ||
              "Forward proxy progress stream failed",
          );
        }
      }
    ...
    if (done) {
      ...
      if (trailingEvent) {
        ...
        if (trailingEvent.type === "error") {
          throw new Error(
            trailingEvent.message ||
              trailingEvent.detail ||
              "Forward proxy progress stream failed",
          );
        }
      }
...
export function fetchSummaryWindows(
  signal?: AbortSignal,
): Promise<SummaryWindowsResponse> {
  return requestJson("/api/summary/windows", { signal });
}

export function fetchPublicMetrics(
  signal?: AbortSignal,
...
export async function fetchPublicLogs(
  token: string,
  limit = 20,
  signal?: AbortSignal,
...
export function fetchApiKeyDetail(
  id: string,
  signal?: AbortSignal,
): Promise<ApiKeyStats> {
...
export function fetchApiKeySecret(
  id: string,
  signal?: AbortSignal,
...
export async function syncApiKeyUsage(id: string): Promise<void> {
  const encoded = encodeURIComponent(id);
  const res = await fetch(`/api/keys/${encoded}/sync-usage`, {
    method: "POST",
  });
  if (!res.ok) {
    let message = "";
    ...
    throw new Error(
      (message ? `${message}` : "Failed to sync key usage") + statusPart,
    );
  }
}
...
export function fetchUserTokens(
  signal?: AbortSignal,
...
export function fetchUserTokenSecret(
  id: string,
  signal?: AbortSignal,
): Promise<UserTokenResponse> {
  const encoded = encodeURIComponent(id);
...
export function probeApiTavilyResearchResult(
  ...
  return requestJsonWithToken(
    `/api/tavily/research/${encodeURIComponent(requestId)}`,
    token,
    {
      method: "GET",
    },
...
export async function addApiKeysBatch(
  ...
  return await requestJson("/api/keys/batch", {
    ...
    body: JSON.stringify({
      ...
      group: trimmedGroup && trimmedGroup.length > 0 ? trimmedGroup : undefined,
      exhausted_api_keys:
        exhaustedApiKeys && exhaustedApiKeys.length > 0
          ? exhaustedApiKeys
          : undefined,
    }),
  });
...
export function fetchKeyMetrics(
  id: string,
  ...
  const encoded = encodeURIComponent(id);
  return requestJson(`/api/keys/${encoded}/metrics?${params.toString()}`, {
    signal,
  });
}
...
export type ValidateAssignedProxyMatchKind =
  | "registration_ip"
  | "same_region"
  | "other";
...
export function fetchKeyLogs(
  ...
  if (since != null) params.set("since", String(since));
  const encoded = encodeURIComponent(id);
  return requestJson(`/api/keys/${encoded}/logs?${params.toString()}`, {
    signal,
  });
}
...
export function fetchKeyLogsPage(
  ...
  const encoded = encodeURIComponent(id);
  return requestJson<ServerRequestLogsPage>(
    `/api/keys/${encoded}/logs/page?${params.toString()}`,
    { signal },
  ).then(normalizeRequestLogsPage);
}
...
export function fetchKeyStickyUsers(
  ...
  const encoded = encodeURIComponent(id);
  return requestJson(`/api/keys/${encoded}/sticky-users?${params.toString()}`, {
    signal,
  });
}

export function fetchKeyStickyNodes(
  id: string,
  signal?: AbortSignal,
): Promise<StickyNodesResponse> {
  const encoded = encodeURIComponent(id);
...
export function fetchRequestLogsPage(
  ...
  appendRequestLogsPageFilters(params, query);
  return requestJson<ServerRequestLogsPage>(`/api/logs?${params.toString()}`, {
    signal,
  }).then(normalizeRequestLogsPage);
}
...
export function fetchRequestLogs(
  ...
  return fetchRequestLogsPage(
    { page, perPage, result, operationalClass },
    signal,
  );
}
...
export function fetchJobs(
  ...
  return requestJson<Paginated<ServerJobLogView>>(
...
export async function updateAdminUserQuota(
  id: string,
  payload: UpdateUserQuotaPayload,
): Promise<void> {
  const encoded = encodeURIComponent(id);
...
export function fetchAdminUserBrokenKeys(
  ...
  return requestJson(`/api/users/${encoded}/broken-keys?${params.toString()}`, {
    signal,
  });
}

export function fetchAdminUserTags(
  signal?: AbortSignal,
): Promise<AdminUserTag[]> {
  return requestJson<{ items: AdminUserTag[] }>("/api/user-tags", {
    signal,
  }).then((response) => response.items);
}

export function createAdminUserTag(
  payload: UpsertAdminUserTagPayload,
): Promise<AdminUserTag> {
...
export function updateAdminUserTag(
  id: string,
  payload: UpsertAdminUserTagPayload,
): Promise<AdminUserTag> {
  const encoded = encodeURIComponent(id);
...
export async function bindAdminUserTag(
  userId: string,
  tagId: string,
): Promise<void> {
...
export async function unbindAdminUserTag(
  userId: string,
  tagId: string,
): Promise<void> {
  const encodedUserId = encodeURIComponent(userId);
  const encodedTagId = encodeURIComponent(tagId);
  await requestNoContent(`/api/users/${encodedUserId}/tags/${encodedTagId}`, {
    method: "DELETE",
  });
}
...
export function fetchTokenSecret(
  id: string,
  signal?: AbortSignal,
): Promise<AuthTokenSecret> {
  const encoded = encodeURIComponent(id);
...
export async function rotateTokenSecret(id: string): Promise<AuthTokenSecret> {
  const encoded = encodeURIComponent(id);
  return await requestJson(`/api/tokens/${encoded}/secret/rotate`, {
    method: "POST",
  });
}
...
export async function createTokensBatch(
  ...
  count: number,
  note?: string,
): Promise<BatchCreateTokensResponse> {
...
export function fetchTokenHourlyBuckets(
  id: string,
  hours = 25,
  signal?: AbortSignal,
): Promise<TokenHourlyBucket[]> {
  const encoded = encodeURIComponent(id);
  const params = new URLSearchParams({ hours: String(hours) });
  return requestJson(
    `/api/tokens/${encoded}/metrics/hourly?${params.toString()}`,
    { signal },
  );
}
...
export function fetchTokenUsageSeries(
  ...
  return requestJson(
    `/api/tokens/${encoded}/metrics/usage-series?${search.toString()}`,
    { signal },
  );
}
...
export function fetchTokenLogsPage(
  ...
  const encoded = encodeURIComponent(id);
  return requestJson<ServerRequestLogsPage>(
    `/api/tokens/${encoded}/logs/page?${params.toString()}`,
    { signal },
  ).then(normalizeRequestLogsPage);
}
...
export interface TokenUsageLeaderboardItem {
  ...
  all_errors: number;
  all_other: number;
  monthly_broken_count: number | null;
  monthly_broken_limit: number | null;
}
...
export function fetchTokenUsageLeaderboard(
  ...
  return requestJson(`/api/tokens/leaderboard?${params.toString()}`, {
    signal,
  });
}
...
export function fetchTokenBrokenKeys(
  ...
  return requestJson(
    `/api/tokens/${encoded}/broken-keys?${params.toString()}`,
    { signal },
  );
}
...
export async function fetchForwardProxySettings(
  signal?: AbortSignal,
): Promise<ForwardProxySettings> {
  const response = await requestJson<ForwardProxySettingsEnvelope>(
    "/api/settings",
    { signal },
  );
...
export function fetchForwardProxyStats(
  signal?: AbortSignal,
): Promise<ForwardProxyStatsResponse> {
